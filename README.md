...
Here are the rules of this challenge.. you must:
 ...
 2) Run `yarn backend` to start the server and get access to API methods such as `getBallotData`. When a user loads the application, we should fetch and display the ballot data.
 3) When you click on a nominee, we should highlight the nominee card and save the selection locally on the client. A user can only select one nominee per category, and we should be able to see all of their selections highlighted. The selected nominee card should follow the style guides below.
 4) If I select my choices for different categories and refresh the page, I should still see my selections highlighted.
 5) Make the layout responsive with at least one breakpoint (your choice as to how it looks on a smaller screen width).
 6) Display a button, that when clicked, displays a sidebar with the latest users' selections
 7) Once the user is finished making their selections, they can click on a submit button, submit the form, and display a results modal screen. A user can dismiss the modal by clicking on the close button (follow the wireframe below).
...
## Requirements:
 ...
 6) Demonstrate use of performance best practices when handling large assets / build size
